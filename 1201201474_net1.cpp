...
int main(int argc, char *argv[])
{
    ...
    FILE *output = fopen("xyz.pcap", "wb");  // Open output file
    if (output == NULL)
    {
    ...
    fwrite((char *)&hdr, sizeof(hdr), 1, output);  // Write original pcap header
    ...
    while (fread((char *)&tt, sizeof(tt), 1, input) && fread((char *)&eth, sizeof(eth), 1, input)) /* Read & Display Timestamp and Ethernet Header Information */
    {
        ...
        // Display the modified source IP address
        cout << "\nModified Source Address: " << ((ip.src_ip >> 0) & 0xFF) << "."
             << ((ip.src_ip >> 8) & 0xFF) << "." << ((ip.src_ip >> 16) & 0xFF) << "."
             << ((ip.src_ip >> 24) & 0xFF) << endl;
